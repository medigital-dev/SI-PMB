    ...
    file: function (context) {
      ...
      this.initialize = function () {
        ...
        if (options.maximumFileSize) {
          let unit = Math.floor(
            Math.log(options.maximumFileSize) / Math.log(1024)
          );
          let readableSize =
        ...
        let body = [
          ...
          '<button href="#" class="btn btn-outline-primary note-file-btn">' +
            lang.file.insert +
            "</button>	",
      ...
      this.createFile = function (url) {
        ...
        let imgBase64RegExp =
          /^data:(image\/jpeg|image\/png|image\/gif|image\/svg|image\/webp).+$/;
        ...
        let videoBase64RegExp =
          /^data:(video\/mpeg|video\/mp4|video\/ogv|video\/webm).+$/;
        ...
        $.each(files, function (idx, file) {
          ...
          if (options.maximumFileSize && options.maximumFileSize < file.size) {
            context.triggerEvent(
              "file.upload.error",
              lang.file.maximumFileSizeError
            );
          } else {
            self
